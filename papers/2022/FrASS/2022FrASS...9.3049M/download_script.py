...
print(query_mdi)
# Note: The results may be paginated if there are too many rows.

# Uncomment the following line to fetch the data
# mdi_files = Fido.fetch(query_mdi)

# Query for SDO/HMI data using a smaller example time range to avoid "Results from 0 Providers"
# Using smaller example time range due to potential issue with broad time range
time_range_hmi_example = a.Time('2010-05-01', '2010-05-31')
query_hmi = Fido.search(time_range_hmi_example, instrument_hmi)
print("SDO/HMI Query Results (Example Time Range):")
print(query_hmi)
