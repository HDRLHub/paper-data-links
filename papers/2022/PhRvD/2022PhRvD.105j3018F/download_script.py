...
# Note: The time range is very broad, which may result in a large number of results and potential pagination issues.
# Using smaller example time range due to potential issues with broad time ranges
celias_query = Fido.search(
    a.Time('1996-01-01', '1996-01-31'),  # Example smaller time range
    a.Instrument('CELIAS'),
    a.Detector('MTOF')
)
...
# Note: The time range is relatively short, so pagination is less likely to be an issue.
